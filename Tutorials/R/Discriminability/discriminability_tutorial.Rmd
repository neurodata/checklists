...
Essentially, we define discriminability with two functions. The first is called the reliability density function, or rdf for short. The reliability density function takes some measure on our data (this could be correlation matrices, for instance), calculates pairwise distances between them for all subjects, and then assigns a score based on how distinguishable the intra-subject measures are (ie, do they have a shorter distance, and are correspondingly more similar?) from the inter-subject measures. Essentially, the score is a mean normalized rank (the method used to be called mnr, and you can see why!) as it gives us a normalized ranking of our intra subject graphs with respect to our inter subject graphs (ie, of the n(subjects) in our dataset, what is the ratio of scans that our intra-subject scans performs better than?). We can see naturally, that this has an upper limit of 1 (all of your intra subject scans are closer to you than the rest of the scans in the dataset), which is what we want to shoot for when using this measure to evaluate pipeline quality. We repeat for all subjects and scans in our dataset, and then report discriminability as the mean of the rdf. 
